...
STATIC VOID_T __uart_recv_handle(TUYA_UART_NUM_E port, VOID_T *data, UINT16_T len)
{
    int i = 0;
    
    TAL_PR_HEXDUMP_INFO("uart rev", data, len);
    
    for(i=0;i<len;i++)
    {
        uart_receive_input(*((uint8_t*)data + i));
    }
}
...
OPERATE_RET tuya_init_last(VOID_T)
{
    ...
    tal_uart_init(USER_UART0, &uart_cfg);
    tal_uart_rx_reg_irq_cb(USER_UART0, __uart_recv_handle);
...
{
    //TAL_PR_DEBUG("app general msg cb: cluster 0x%02x, cmd %d", msg->cluster, msg->command);
...
TAL_MSG_RET_E tal_zcl_specific_msg_recv_callback(TAL_ZCL_MSG_T *msg)
{
    ...
    TAL_PR_DEBUG("app specific msg cb: cluster 0x%02x, cmd 0x%02x, mode 0x%02x", msg->cluster, msg->command, msg->mode);
    ...
    {
        //user define here!
        TAL_PR_HEXDUMP_INFO("zigbee rev", msg->payload, msg->length);
        pho_cluster_handler(msg->command, msg->payload, msg->length, app_cmd_type);
        break;
    }
    default:
        break;
